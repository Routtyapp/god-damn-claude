...
# 4.1 기본 금지 패턴 (사용자 요청 없이 절대 추가 금지)

다음 컴포넌트는 **사용자가 명시적으로 요청한 경우에만** 사용한다.
디자인 완성도를 높이거나 "있으면 좋을 것 같아서" 임의로 추가하지 않는다.

## Announcement Pill / Hero Badge

```
• 🎉 Series B 펀딩 완료 — $40M 투자 유치
```

- 히어로 헤드라인 위에 올라가는 pill 형태의 공지 배지
- 요청 없이 추가하면 실제 공지 내용과 무관한 장식이 됨
- **요청 키워드**: "announcement", "hero badge", "pill 배지", "공지 배너", "상단 알림"

## Top Banner / Sitewide Notice Bar

- 전체 페이지 최상단을 가로지르는 공지 바
- 프로모션·시스템 점검 등 실제 공지 목적 외 사용 금지

## Ticker / Marquee

- 텍스트가 가로로 흐르는 자동 슬라이드 배너
- 접근성 문제(prefers-reduced-motion), 가독성 저하 위험
