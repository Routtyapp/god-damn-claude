...
5. **Affordance First**
   - 모든 인터랙티브 컴포넌트는 목적을 시각적으로 명시한다.
   - 사용자는 클릭/탭 전에 "이게 무엇을 하는가"를 알아야 한다.
   - affordance는 호버 이전에도 가시적이어야 한다 (hidden 금지).
   - 예: 리다이렉트 카드 → 이동 아이콘 필수 / 삭제 버튼 → danger 톤 필수

6. **DRY Component First**
   - 새 컴포넌트를 만들기 전에 기존 공용 컴포넌트를 먼저 확인한다.
   - 동일하거나 유사한 UI가 2곳 이상에서 사용되면 즉시 공용으로 추출한다.
   - 코드 중복은 유지보수 비용이다 — 같은 버그를 여러 곳에서 고치게 된다.
...
## 3.5 Intent Indicator (인터랙티브 목적 표시)

인터랙티브 컴포넌트는 목적에 해당하는 시각적 단서를 반드시 포함한다.

| 컴포넌트 목적 | 필수 affordance | 권장 위치 |
|---|---|---|
| 외부/내부 리다이렉트 | 이동 아이콘 (`→`, `↗`) | 우측 상단 또는 우측 하단 |
| 삭제·파괴적 액션 | danger 톤 + 아이콘 | 액션 영역 |
| 확장·펼침 | chevron 아이콘 | 우측 끝 |
| 다운로드 | 다운로드 아이콘 | 액션 영역 |
| 선택 가능한 카드 | hover 배경 변화 + `cursor: pointer` | 컴포넌트 전체 |

### 규칙

- 아이콘 단독 사용 시 `aria-label` 필수
- affordance는 호버 전에도 가시적이어야 한다 — opacity 낮춰도 되지만 `display: none` 금지
- 여정을 유도하는 CTA는 micro-interaction으로 강화한다 → micro-interaction skill 참조
...
src/ui/
├── tokens/
├── primitives/   # AppButton, AppInput 등 — 재사용 최소 단위
├── patterns/     # Form, Table, EmptyState — 조합 패턴
...
| 상황 | 액션 |
|---|---|
| 동일 컴포넌트를 2곳 이상에서 사용 | `ui/primitives/` 또는 `ui/patterns/`로 이동 |
| 스타일만 다르고 구조가 같은 컴포넌트 | props로 변형 처리 후 공용화 |
| 같은 로직의 훅이 여러 feature에 존재 | `src/hooks/`로 이동 |
| feature 전용이지만 다른 feature가 필요로 함 | `ui/`로 승격 |

## 공용화 절차

1. feature 내부에서 먼저 구현한다
2. 2번째 사용처가 생기는 시점에 공용으로 추출한다 (YAGNI 원칙)
3. `ui/index.ts`에 export를 추가한다
4. 원래 import 경로를 `@/ui/...`로 교체한다

## 레벨별 위치

```
재사용 빈도 낮음 → features/{domain}/components/  (도메인 전용)
재사용 빈도 중간 → ui/patterns/                   (조합 패턴)
재사용 빈도 높음 → ui/primitives/                  (기본 단위)
```

## 금지

- 같은 컴포넌트를 두 feature에 각각 복사 붙여넣기
- `features/A`에서 `features/B`의 컴포넌트를 직접 import
  → 반드시 `ui/`로 추출 후 공유
